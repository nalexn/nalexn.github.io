...
The local screen's state, which is not shared with other screens, can safely be stored in the screen module itself. This only depends on the [architecture](https://medium.com/ios-os-x-development/ios-architecture-patterns-ecba4c38de52) you use for you screen module. For example, in case of [`Model-View-Controller`](https://ru.wikipedia.org/wiki/Model-View-Controller) this is the `ViewController`, for [`Model-View-ViewModel`](https://en.wikipedia.org/wiki/Model-view-viewmodel) it would be `Model`.

Things get much more complicated when we have the data meant to be transmitted or shared by multiple modules. 
...
There are many ways to comply with all three patterns at once. One example is [Redux](https://github.com/reduxjs/redux) library, initially created for JavaScript world. For iOS we have [ReSwift](https://github.com/ReSwift/ReSwift), with is my personal favorite.
...
On one hand, we all need to aim for simplicity of the code we write, but there are shortcuts engineers should not take, and one of them is neglecting the proper state management in their apps.